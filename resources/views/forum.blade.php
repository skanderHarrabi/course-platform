...
<article id="post-9" class="post-9 page type-page status-publish hentry">
        ...
        @foreach($discussions as $di)
        <div class="panel panel-default">
                <div class="panel-heading ">
                   <img src="{{$di->user->profile->avatar}}" alt="" width="40px" height="40px">&nbsp;&nbsp;&nbsp;
                <span>{{$di->user->name}}, <b>{{$di->created_at->diffForHumans()}}</b></span> 
                   <a href="{{route('discussionshow',['id'=>$di->id])}}" class="btn btn-default pull-right">View</a>
                </div>
                <div class="panel panel-body">
                     ...
                     <p class="text-center">
                         {{str_limit($di->content,50)}}
                     </p>
                </div>
                <div class="panel-footer">
                    <span>
                        {{$di->replies->count()}} Replies

                    </span>
                    <a href="{{route('formation.descussion',['slug'=>$di->formation->slug])}}" class="pull-right btn btn-default btn-xs">{{$di->formation->title}} </a> <span class="pull-right"> See all descussion of the cource :</span>
                </div>
            </div>
        @endforeach
